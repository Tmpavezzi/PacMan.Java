   ...
   private Booster booster = new Booster(400, 400);
	...
	private void run() {
		while (player.getLife() > 0) {
			if(player.isInvencible())invencibleCount --;
			if(invencibleCount==0)player.setInvencible(false);
            ...
            if((player.colide(ghost1)||player.colide(ghost2)
            || player.colide(ghost3)|| player.colide(ghost4)
            || player.colide(bomb))&&
			!player.isInvencible()){
                player.damage();
            }
			...
			if(player.colide(booster)){
				...
				invencibleCount = booster.getDuration();
			}
